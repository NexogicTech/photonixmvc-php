...
class Routes
{
    ...
    protected static function dispatchToController(string $controller, string $path = '', array $params = []): void
    {
        ...
        if (!class_exists($className) && $path === '') {
            $auto = self::resolveClassPath($className);
            if ($auto !== null) {
                require_once $auto;
            }
        }
        ...
        if (!class_exists($className)) {
            http_response_code(500);
            \PhotonixCore\Logger::error('Class not found', ['class' => $className, 'controller' => $controller]);
            echo self::renderError('error');
            return;
        }
        ...
        if (!method_exists($instance, $methodName)) {
            http_response_code(500);
            \PhotonixCore\Logger::error('Method not found', ['class' => $className, 'method' => $methodName]);
            echo self::renderError('error');
            return;
        }
            ...
            if (count($parameters) === 0) {
                ...
                if (count($parameters) === 1 && ($first->getName() === 'params' || $firstIsArray)) {
                    ...
                    foreach ($parameters as $p) {
                        ...
                        if ($name === 'request' || $type === Request::class || $type === '\\' . Request::class) {
                            $orderedArgs[] = $requestObj;
...
//
           if (is_string($result)) {
            ...
            http_response_code(500);
            \PhotonixCore\Logger::error('Controller exception', ['controller' => $controller, 'error' => $e->getMessage(), 'file' => $e->getFile(), 'line' => $e->getLine()]);
            echo self::renderError('error');
            return;
        }
    }

    protected static function resolveClassPath(string $className): ?string
    {
        ...
        $rel = '/' . implode('/', array_slice($parts, 1)) . '.php';
        $exact = $root . $rel;
        if (is_file($exact)) {
            return $exact;
        }
        $lower = '/' . strtolower(implode('/', array_slice($parts, 1))) . '.php';
        $lowerPath = $root . $lower;
        if (is_file($lowerPath)) {
            return $lowerPath;
        }
        $fullLower = '/' . strtolower(implode('/', $parts)) . '.php';
        $fullLowerPath = $root . $fullLower;
        if (is_file($fullLowerPath)) {
            return $fullLowerPath;
        }
        return null;
    }
